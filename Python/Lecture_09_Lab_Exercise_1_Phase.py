...
# plt.quiver(X, np.zeros_like(X), UNormalized, V, color="b")
# plt.xlabel("x")
# plt.ylabel("dx/dt")
...
xVals = np.linspace(-3, 3, 50)  # Range of x values.
yVals = np.linspace(-3, 3, 50)  # Range of y values.
X, Y = np.meshgrid(xVals, yVals)  # Create a grid.

# Compute the vector field.
params = (0.1, 0.7, 0.8, 0.5)  # Example parameters for FitzHugh-Nagumo.
